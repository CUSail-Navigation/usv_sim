...
def set_sailboat_heading(pub_state):
    ...
    quaternion = (current_state.pose.pose.orientation.x, current_state.pose.pose.orientation.y,
                  current_state.pose.pose.orientation.z, current_state.pose.pose.orientation.w)

    euler = tf.transformations.euler_from_quaternion(quaternion)

    quaternion = tf.transformations.quaternion_from_euler(
        euler[0], euler[1], math.radians(current_heading))
...
def talker():
    ...
    pub_sail = rospy.Publisher(
        '/sailboat/joint_setpoint', JointState, queue_size=10)
