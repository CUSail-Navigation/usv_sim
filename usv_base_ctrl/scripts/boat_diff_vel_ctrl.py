...
class VelocityCtrl():
    ...
    def __init__(self):
        ...
        self.pub_motor = rospy.Publisher(
            'thruster_command', JointState, queue_size=10)

        rospy.Subscriber("state", Odometry, self.get_usv_vel)
        rospy.Subscriber("cmd_vel", Twist, self.get_target_vel)

        while not rospy.is_shutdown():
            self.pub_motor.publish(self.thruster_ctrl_msg(
                self.vel_ctrl(), thruster_name))
            r.sleep()
    ...
    def vel_ctrl(self):
        ...
        if self.target_vel.angular.z == 0:
            ...
            self.ang_vel = self.ang_vel * \
                self.kp_ang + self.I_ang(self.ang_vel)
        ...
        msg = "atual: {0}; desejada: {1}; erro: {2}; vel_left: {3}; vel_right: {4}; erro_bruto: {5}" .format(
            self.usv_vel.twist.twist.angular.z, self.target_vel.angular.z, self.ang_vel, self.vel_left, self.vel_right, self.erro)
        rospy.loginfo(msg)
