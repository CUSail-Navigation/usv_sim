...
def initilizeArray():
    ...
    barrierImage = matplotlib.pyplot.imshow(
        bImageArray, origin='upper', interpolation='none')
...
def loadColisions():
    ...
    if (len(obstaculos) == len(array)) and (len(obstaculos[0]) == len(array[0])):
        ...
        print(obstacle_image, ": (", len(obstaculos),
              ", ", len(obstaculos[0]), ")")
    else:
    ...
    print("height: ", height)
    print("width: ", width)
    data = numpy.zeros((len(obstaculos), len(
        obstaculos[0]), 3), dtype=numpy.uint8)
    data[:] = [255, 255, 255]
