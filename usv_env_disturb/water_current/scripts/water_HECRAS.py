...
def preprocessDataset2():
    ...
    print("Preprocessing the dataset indexes(length: ",
          dataSetLen, "). This can take some minutes...")
...
def handleWaterCurrent2(req):
    ...
    print("Received request", req.x, req.y, " --> ",
          datasetX[time][i], datasetY[time][i])
...
def loadMap():
    ...
    rospy.logerr(
        "-################### Preparing map to rviz. Size (%d, %d) origin(%d, %d)", width, height, 0, 0)
